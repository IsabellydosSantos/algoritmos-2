...
def exibir_hastes_premium(hastes, n, movimentos, minimo_teorico, start_time):
    """Exibição ultra-detalhada do jogo"""
    
    tamanho_maximo = n
    largura_total = 88  # Aumentado para acomodar discos maiores
    
    # Moldura superior (ajustada para largura 88)
    print("\n" + "╔" + "═" * largura_total + "╗")
    print("║" + " 🎮 TORRE DE HANÓI - EDIÇÃO PREMIUM (8 DISCOS) 🎮 ".center(largura_total) + "║")
    print("╠" + "═" * largura_total + "╣")
    
    # Objetivo
    print(f"║ 🎯 Objetivo: Mover {n} discos de A → C".ljust(largura_total + 1) + "║")
    print("╠" + "═" * largura_total + "╣")
    
    # Cabeçalho das hastes (com discos 8 na legenda)
    header = "║     "
    hastes_nomes = {'A': '🔴 A', 'B': '🟡 B', 'C': '🟢 C'}
    largura_haste = 24  # Aumentado para discos maiores
    for haste in ['A', 'B', 'C']:
        header += f"{hastes_nomes[haste]}".center(largura_haste)
    print(header + "      ║")
    
    print("╠" + "─" + "─" * (largura_total - 1) + "╣")
    ...
    for nivel in reversed(niveis):
        ...
        for disco in nivel:
            if disco == 0:
                # Haste vazia
                espaco = " " * (tamanho_maximo * 2 - 1)
                linha += f"{espaco}".center(largura_haste)
            else:
                disco_str = disco_colorido(disco, tamanho_maximo)
                # Centralizar o disco
                largura_disco = tamanho_maximo * 2 - 1
                disco_centralizado = disco_str.center(largura_disco)
                linha += f"{disco_centralizado}".center(largura_haste)
        print(linha + "     ║")
    ...
    print(f"║ 📦 Movimentos: {movimentos:<3}  ", end="")
    print(f"🎯 Mínimo: {minimo_teorico:<3}  ", end="")
    print(f"📊 Progresso: {progresso:.0f}%".ljust(35) + "║")
    
    # Barra de progresso visual (ajustada para nova largura)
    barras = int(progresso / 100 * 50)  # Aumentado de 40 para 50
    barra_progresso = "█" * barras + "░" * (50 - barras)
    print(f"║ [{barra_progresso}] {progresso:.0f}%".ljust(largura_total + 1) + "║")
    
    print(f"║ 💯 Eficiência: {eficiencia:.0f}%  ", end="")
    print(f"⏱️  Tempo: {minutos:02d}:{segundos:02d}".ljust(55) + "║")
    
    # Dica inteligente
    dica = gerar_dica_premium(hastes, n)
    print(f"║ 💡 Dica: {dica:<76} ║")
    
    # Recorde se houver
    print("╠" + "═" * largura_total + "╣")
    
    # Rodapé com controles
    print("║ Comandos: A/B/C para haste | Q para desistir".ljust(largura_total + 1) + "║")
    print("╚" + "═" * largura_total + "╝")
...
def jogar_hanoi_premium():
    ...
    try:
        ...
        while hastes['C'] != list(range(n, 0, -1)):
            ...
            # Input do usuário
            print()
            comando = input("🎮 Digite a haste de origem e destino (ex: A B) ou Q para sair: ").upper().strip()
            ...
            try:
                ...
                print("\n❌ Comando inválido! Use: A B (origem e destino separados por espaço)")
                time.sleep(1.5)
                limpar_tela()
